...
/*   Updated: 2024/08/02 16:08:00 by jsarda           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	read_map(int fd, t_prog *data, char *map_name)
{
	...
	if (!data->map_data.map_str)
		return (ft_errors(strerror(errno), errno));
	fd = open(map_name, O_RDONLY);
	if (fd == -1)
		return (ft_errors("Error opening file", 1));
...
void	init_map(t_prog *data)
{
	...
	if (ft_strlen(map_name) <= 4 || !ft_strnstr(map_name + (ft_strlen(map_name)
				- 4), ".cub", 5))
		return (ft_errors("File format must be 'exemple.cub' \
		or file does not exist", 1));
	fd = open(map_name, O_RDONLY);
	if (fd == -1)
		return (ft_errors("Error opening file", 1));
